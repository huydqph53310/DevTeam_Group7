...
<html lang="vi">
	<head>
		...
		<link rel="canonical" href="https://ego-mobile.mysapo.net/he-thong-cua-hang"/>
...
<meta name="description" content="https://docs.google.com/spreadsheets/d/e/2PACX-1vSWqVkCPUwFr8Bz9XMwLFaVKWdH5yBdFnef1WA4wrKcySRkh3AZyTx0ibxhXtF9SPd_M-e8t_ZxC5T5/pub?gid=0&single=true&output=csv">
		<title>Hệ thống cửa hàng		</title>
		<meta name="keywords" content="Hệ thống cửa hàng, Ego Mobile, ego-mobile.mysapo.net"/>		
...
<meta property="og:description" content="https://docs.google.com/spreadsheets/d/e/2PACX-1vSWqVkCPUwFr8Bz9XMwLFaVKWdH5yBdFnef1WA4wrKcySRkh3AZyTx0ibxhXtF9SPd_M-e8t_ZxC5T5/pub?gid=0&single=true&output=csv">
<meta property="og:url" content="https://ego-mobile.mysapo.net/he-thong-cua-hang">
...
<link rel="preload" as='style' type="text/css" href="//bizweb.dktcdn.net/100/507/051/themes/936909/assets/blog_article_style.scss.css?1731407153286">
<link href="//bizweb.dktcdn.net/100/507/051/themes/936909/assets/blog_article_style.scss.css?1731407153286" rel="stylesheet" type="text/css" media="all" />



<link rel="preload" as="style" type="text/css" href="//bizweb.dktcdn.net/100/507/051/themes/936909/assets/style-maps.scss.css?1731407153286" />
<link href="//bizweb.dktcdn.net/100/507/051/themes/936909/assets/style-maps.scss.css?1731407153286" rel="stylesheet" type="text/css" media="all" />
...
<script>
	...
	meta.customer = { "id": 16237204, "first_name": "huy", "last_name": "do",  "phone": "84977675028", "email": "admin.runsytem.mod@huydev.id.vn" };
...
<script type="application/ld+json">
      ...
      
      {
                ...
                "item": 
                {
                  "@id": "https://ego-mobile.mysapo.net/he-thong-cua-hang",
...
<section class="page-he-thong">
	<div class="container margin-top-20 margin-bottom-20 ">
		<div class="clear-fix white">
			<div class="module-title"><h1 class="title-module">Hệ thống cửa hàng</h1></div>
			<div class="cssload-loader">
				Đang tải bản đồ...
			</div>
			<div class="sectionContentStore row" style="display:none;">
				<div class="col-12 col-lg-4">
					<div class="leftCollumStore option-chos">
						<div class="form-group group-option">
							<label class="select-city-wrapper group-city">
								<select name="select-city" class="select-city form-control">
									<option class="option-chos" value="" disabled selected>Chọn Tỉnh/ Thành Phố</option>
								</select>
							</label>
							<label class="select-city-wrapper group-district">
								<select name="select-district" class="select-district form-control">
									<option class="option-chos" value="" disabled selected>Chọn quận huyện</option>
								</select>
							</label>
						</div>
						<div class="resultStore">
							<div id="list-store" class="info-store"></div>
						</div>
					</div>
				</div>
...
<script>
	...
	var $liststore = document.querySelector('#list-store');
	...
	function csvToJSON(csv) {
		const rows = csv.split(/\r?\n/g).map(function (x) {
			return [...x.matchAll(/(?:"([^"]+?)"|([^,]+?)|(,))(?=,|$)/g)].map(function ([, quoted, x]) {
				return quoted || x || "";
			});
		});
	...
	function groupArrayByKey(json, targets) {
		let result = [];
		for (const item of json) {
			const index = result.findIndex(function (rsItem) {
				let exits = true;
				for (const target of targets) {
					if (rsItem[target].trim() !== item[target].trim()) {
						exits = false;
						break;
					}
				}
			...
			for (const target of targets) {
				delete itemWithOutKey[target];
			}
			if (index > -1) {
				result[index].data.push(itemWithOutKey);
			} else {
				result.push({
					...targets.reduce(function (object, target) {
					object[target] = item[target].trim();
					return object;
				}, {}),
					data: [itemWithOutKey],
			});
	...
	function downloadCsv(options) {
		if (!options.url || !options.callback) {
			console.error("Missing url or callback");
			return;
		}
		fetch(options.url, {
			method: "get",
			...
			options.callback(options.group_by ? groupArrayByKey(json, options.group_by) : json);
		});
	}

	downloadCsv({
		url: theme.settings.liststore,
		group_by: ["thanh_pho"],
		callback: function (data) {
			callbackData = data;
			renderMap(data);
			document.querySelector('.cssload-loader').style.display = 'none';
			document.querySelector('.sectionContentStore').style.display = 'flex';
	...
	downloadCsv({
		...
		group_by: ["quan_huyen"],
		callback: function (data) {
			callbackDataDistrict = data;
			renderMapDistrict(data);
		},
	});

	function renderMapDistrict(data) {
		var cityToFilter = defaultCity;
		var filteredDistricts = data.filter(function (value) {
			return value.data[0].thanh_pho === cityToFilter;
		});

		var uniqueDistricts = [...new Set(filteredDistricts.map(item => item.quan_huyen))];

		uniqueDistricts.forEach(function (district, index) {
	...
	function renderMap(data) {
		...
		data.forEach(function (value) {
			var $option = document.createElement("option");
			$option.value = value.thanh_pho;
			$option.textContent = value.thanh_pho;

			if (value.thanh_pho === defaultCity) {
		...
		setTimeout(function(){
			callbackDataDistrict.forEach(function (value) {

				if (value.data[0].thanh_pho === defaultCity) {
					value.data.forEach(function (item) {
						renderItemStore(item);
					});

					var $firstItem = $liststore.querySelector('.item:first-child');
					$firstItem.classList.add('checked');
					BaseGlobal.lazyloadImage(theme.settings.lazyload);
					setTimeout(function(){
						document.getElementById('map').innerHTML = $firstItem.getAttribute('data-frame').replace(/"/g, "");
					},500)
				}
			});
		}, 400)
	...
	function renderItemStore(item){
		const $item = document.createElement('div');
		$item.className = 'item item-location';
		var phonetrim = item.phone.replaceAll(' ','');
		$item.dataset.frame = item.code.replace(/"/g, "");
		$item.innerHTML = `
			<div class="left-lc">
				<h4>${item.name}</h4>
				<p>Địa chỉ: ${item.dia_chi}</p>
				<p>Hotline: <a href="tel:${phonetrim}">${item.phone}</a></p>
				<a class="view-link" target="_blank" href="${item.link_map}" title="Xem trên Google Maps">Xem trên Google Maps</a>
					</div>
				<div class="right-lc">
				<img class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=" data-src="${item.image_store}" alt="${item.thanh_pho} - ${item.quan_huyen}"/>
			</div>
		`;
		$liststore.appendChild($item);
	}
	...
	$selectcity.addEventListener('change', function () {
		...
		var filteredDistricts = callbackDataDistrict.filter(function (value) {
			return value.data[0].thanh_pho === selectedCity;
		});
		var uniqueDistricts = [...new Set(filteredDistricts.map(item => item.quan_huyen))];
		...
		callbackDataDistrict.forEach(function (value) {
			...
			if (value.data[0].thanh_pho === selectedCity) {
				value.data.forEach(function (item) {
					renderItemStore(item);
				});
	...
	$selectdistrict.addEventListener('change', function () {
		...
		callbackDataDistrict.forEach(function (value) {
			...
			if (value.quan_huyen === selectedDistrict) {
				value.data.forEach(function (item) {
					renderItemStore(item);
				});
	...
	document.addEventListener('click', function (e) {
		var targetElement = e.target.closest('.item');
		if (targetElement) {
			e.preventDefault();
			...
			document.getElementById('map').innerHTML = targetElement.dataset.frame;
		}
	});
